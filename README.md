...
当前版本主要针对实际使用中的 WorkBuddy 国际版接入、云端 OAuth 登录、OpenAI 兼容调用、DeepSeek V4.1 Flash 深度思考与流式响应等场景进行了继续适配和修正。

部分问题定位、协议排查、代码修改与文档整理过程中使用了 ChatGPT 作为开发辅助工具；仓库维护者负责需求判断、测试验证、最终修改与发布。开发过程中既有有效补丁，也有试错与回退，相关过程均保留在提交历史中。
